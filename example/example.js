...
async function run () {
  ...
  const archive = DDrive('My archive name', {
  ...
  const url = `dweb://${archive.key.toString('hex')}`
  ...
  // This example is currently broken because dBrowser's  website isn't on dWeb 3  yet
  const key = await resolveName('dweb://dbrowser.com')

  console.log('Resolved key', key)

  const SOME_URL = 'dweb://0a9e202b8055721bd2bc93b3c9bbc03efdbda9cfee91f01a123fdeaadeba303e/'

  const someArchive = DDrive(SOME_URL)
  ...
  // Pass in dDatabases from the SDK into other dweb data structures
